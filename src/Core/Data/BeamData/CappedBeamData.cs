...
namespace BeamSizing.Data
{
    ...
    public static class CappedBeamData
    {
        ...
        /// <summary>
        /// Helper method to find capped beam by W-shape and channel combination
        /// </summary>
        /// <param name="wShape">W-shape designation (e.g., "12x40")</param>
        /// <param name="channel">Channel designation (e.g., "12x20.7")</param>
        /// <returns>CappedBeamProperties if found, null otherwise</returns>
        public static CappedBeamProperties? FindByComponents(string wShape, string channel)
        {
            string designation = $"{wShape}+{channel}";
            return CappedBeams.Find(b => b.Designation.Equals(designation, StringComparison.OrdinalIgnoreCase));
        }

        /// <summary>
        /// Get all available channel types
        /// </summary>
        /// <returns>List of unique channel designations</returns>
        public static List<string> GetAvailableChannels()
        {
            var channels = new HashSet<string>();
            foreach (var beam in CappedBeams)
            {
                channels.Add(beam.Channel);
            }
            return new List<string>(channels);
        }

        /// <summary>
        /// Get all W-shapes that can be combined with a specific channel
        /// </summary>
        /// <param name="channel">Channel designation</param>
        /// <returns>List of compatible W-shapes</returns>
        public static List<string> GetCompatibleWShapes(string channel)
        {
            var wShapes = new List<string>();
            foreach (var beam in CappedBeams)
            {
                if (beam.Channel.Equals(channel, StringComparison.OrdinalIgnoreCase))
                {
                    // Extract W-shape from designation (remove "+channel" part)
                    string wShape = beam.Designation.Split('+')[0];
                    wShapes.Add(wShape);
                }
            }
            return wShapes;
        ...
        public static List<CappedBeamProperties> GetBeamsInWeightRange(double minWeight, double maxWeight)
        {
            return CappedBeams.Where(b => b.TotalWeight >= minWeight && b.TotalWeight <= maxWeight)
                             .OrderBy(b => b.TotalWeight)
                             .ToList();
        }

        /// <summary>
        /// Validates that the capped beam data is complete and consistent
        /// </summary>
        /// <returns>True if data validation passes</returns>
        public static bool ValidateData()
        {
            var beamCount = CappedBeams.Count;
            var channels = GetAvailableChannels();

            Console.WriteLine($"Capped beam data validation:");
            Console.WriteLine($"  Total beam combinations: {beamCount}");
            Console.WriteLine($"  Available channels: {string.Join(", ", channels)}");

            // Check for duplicate designations
            var designations = CappedBeams.Select(b => b.Designation).ToList();
            var duplicates = designations.GroupBy(d => d).Where(g => g.Count() > 1).Select(g => g.Key);

            if (duplicates.Any())
            {
                Console.WriteLine($"Data validation failed: Duplicate designations found: {string.Join(", ", duplicates)}");
                return false;
            }

            Console.WriteLine($"Capped beam data validation passed: {beamCount} combinations loaded successfully");
            return true;
        }
    }
}